...
document.getElementById('store-registration-form')?.addEventListener('submit', (e) => {
    ...
    const slug = slugInput.value; // Value is already cleaned by input listener, but we check again

    // --- NEW: Slug Validation (No Symbols/Spaces) ---
    // This Regex allows ONLY alphanumeric characters (a-z, A-Z, 0-9)
    const slugRegex = /^[a-zA-Z0-9]+$/;

    if (!slugRegex.test(slug)) {
        showNotification('⚠️ Invalid Store URL. Please use letters and numbers only (no spaces or symbols).');
        slugInput.focus();
        slugInput.style.border = "2px solid red"; // Visual cue
        setTimeout(() => slugInput.style.border = "", 3000); // Reset after 3s
        return;
    }
    // ------------------------------------------------
    ...
    auth.createUserWithEmailAndPassword(email, pass)
        // ... (The rest of your existing registration code remains the same from here)
        .then((cred) => {
...
function renderProductsPage(page) {
    ...
    // Render Page Numbers (Max 5 on desktop, Max 3 on mobile)
...
async function loadStoreDetail(slug) {
    ...
    try {
        ...
        // STEP 1: Try to fetch all approved stores (this always works for everyone)
        ...
        if (!storeDoc) {
            container.innerHTML = `
                <div class="store-empty-state">
                    ...
                    <p>The store you are looking for does not exist or is not available.</p>
        ...
        const productsSnap = await db.collection('products').where('storeId', '==', resolvedStoreId).get();
        const products = [];
